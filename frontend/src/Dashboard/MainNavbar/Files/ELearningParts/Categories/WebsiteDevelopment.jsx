...
function Graphic() {
  ...
  const token = useSelector((state) => state.auth.token); // ✅ Token for delete
  ...
  return (
    <div className="font-[Chivo] bg-blue-100 py-10 px-6 sm:px-10 lg:px-24">
      ...
      <div className="flex justify-center pt-6">
        ...
        {Array.from(
          { length: Math.ceil(graphicCourses.length / postsPerPage) },
          (_, i) => (
            <button
              key={i}
        ...
        <button
          ...
          onClick={() =>
            setCurrentPage(
              currentPage === Math.ceil(graphicCourses.length / postsPerPage)
                ? currentPage
                : currentPage + 1
            )
          }
          disabled={currentPage === Math.ceil(graphicCourses.length / postsPerPage)}
        >
          <FaChevronRight />
        </button>
      </div>
