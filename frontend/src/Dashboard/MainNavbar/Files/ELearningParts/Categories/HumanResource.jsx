...
function HumanResource() {
  ...
  const humanCourses = courses.filter(
    (course) => course.category === 'Human Resource'
  );

  const indexOfLastCourse = currentPage * postsPerPage;
  const indexOfFirstCourse = indexOfLastCourse - postsPerPage;
  const currentCourses = humanCourses.slice(indexOfFirstCourse, indexOfLastCourse);
  ...
  return (
    <div className="font-[Chivo] bg-blue-100 py-10 px-6 sm:px-10 lg:px-24">
      ...
      <div className="flex justify-center pt-6">
        ...
        {Array.from(
          { length: Math.ceil(humanCourses.length / postsPerPage) },
          (_, i) => (
            <button
              key={i}
        ...
        <button
          ...
          onClick={() =>
            setCurrentPage(
              currentPage === Math.ceil(humanCourses.length / postsPerPage)
                ? currentPage
                : currentPage + 1
            )
          }
          disabled={currentPage === Math.ceil(humanCourses.length / postsPerPage)}
        >
          <FaChevronRight />
        </button>
      </div>
